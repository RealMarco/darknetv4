# -*- coding: utf-8 -*-
"""
Created on Thu Oct 13 15:51:38 2022
@author: Marco
Project: YOLO
"""
...
def convert_annotation_and_crop_objects(image_id):   # convert .xml annotations to .txt ones
    ...
    if os.path.isfile('RPdevkit/RP2022/JPEGImages/%s.png' %image_id):
        img = cv2.imread('RPdevkit/RP2022/JPEGImages/%s.png' %image_id)
    elif os.path.isfile('RPdevkit/RP2022/JPEGImages/%s.jpg' %image_id):
        img = cv2.imread('RPdevkit/RP2022/JPEGImages/%s.jpg' %image_id)
    elif os.path.isfile('RPdevkit/RP2022/JPEGImagesTest/%s.jpg' %image_id):
        img = cv2.imread('RPdevkit/RP2022/JPEGImagesTest/%s.jpg' %image_id)
    else:
        img = cv2.imread('RPdevkit/RP2022/JPEGImagesTest/%s.png' %image_id)
    if os.path.isfile(in_file_path):  # with .xml annotations, convert and write the content
        ...
        for obj in root.iter('object'):
            ...
            # crop images from .xml labels and save objects to folders named by their classes
            cropped_img = img[int(xmlbox.find('ymin').text):int(xmlbox.find('ymax').text),int(xmlbox.find('xmin').text):int(xmlbox.find('xmax').text)]
            cv2.imwrite('RPdevkit/RP2022/CroppedJPEGImages/'+cls+'/'+image_id+cls+'.jpg', cropped_img)
...
if not os.path.isdir(work_sapce_dir):
    os.mkdir(work_sapce_dir)
annotation_dir = os.path.join(work_sapce_dir, "Annotations/") # store .xml labels for both training and testing
...
if not os.path.isdir(RP_file_dir):
        os.mkdir(RP_file_dir)
cropped_image_dir = os.path.join(work_sapce_dir, "CroppedJPEGImages/")  # store cropped images by classes for both training and testing
if not os.path.isdir(cropped_image_dir ):
        os.mkdir(cropped_image_dir)
clear_hidden_files(cropped_image_dir)
for class_name in classes:
    class_dir = os.path.join(cropped_image_dir, class_name)
...
RP_train_file.close()
RP_test_file.close()
if not os.path.exists('RPdevkit/RP2022/labels'):  # # store .txt labels for both training and testing
...
for i in range(0,len(list)):
    ...
    if os.path.isfile(path):
        ...
        convert_annotation_and_crop_objects(nameWithoutExtention)  # convert .xml into .txt
...
for i in range(0,len(test_list)):
    ...
    if os.path.exists(test_annotation_path):
        ...
        convert_annotation_and_crop_objects(test_nameWithoutExtention)
